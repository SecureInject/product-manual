...
本アプリは個人製作のためGoogle Playストア外で配布されるアプリ（APKファイル）です。以下の手順でインストールを行ってください。

### Step 1: APKファイルのダウンロード
BOOTHの「SecureInject 制御用ソフトウェア（AndroidアプリAPK）」ページから「SecureInject_vx.x.x.zip」をダウンロードします。
（BOOTHページ：　https://secureinject.booth.pm/　）

スマートフォンからの標準インストール

ステップ1: ZIPファイルの解凍</br>
１．BOOTHから無料ダウンロード後、「←ダウンロード」メニューで「SecureInject_vx.x.x.zipを解凍しますか？」と表示されるので「解凍」を選択します。</br>
２．「SecureInject_vx.x.x.zipを解凍しました。」と表示されるので、ZIPファイルの削除にチェックを入れ（任意）、「完了」を選択します。</br>
　※これにより、アプリのインストールファイルがDownloadフォルダに解凍されます。</br>
</br>
ステップ2: インストーラーの起動</br>
• SecureInject_vx.x.x.apk がスマホ内部ストレージの「←ダウンロード」メニューの上部の「すべて」、もしくは「SecureInject_vx.x.x」を選択すると、SecureInject_vx.x.x.apk がアイコンと共に表示されるのでクリック（タップ）します。</br>
</br>
ステップ3: セキュリティ許可の設定（Files by Google）</br>
１．Files by Googleの警告で「セキュリティ上の理由からお使いのスマートフォンでは現在、この提供先からの不明なアプリをインストールすることはできません。これは[設定]で変更できます。」と表示されるので、「設定」を選択します。</br>
２．「不明なアプリのインストール」画面に移動するので、「この提供元のアプリを許可」を有効（オン）にします。</br>
</br>
ステップ4: インストールの完了とホーム画面への追加</br>
１．画面にSecureInjectの「このアプリをインストールしますか？」と表示されるので、「インストール」を選択します。</br>
２．インストールが完了したら、メニュー画面で下から上にスワイプし、アプリの一覧を開きます。SecureInjectがインストールされていますので、そのままアイコンをドラッグし、上方向にスワイプしてホーム画面に追加してください</br>
---</br>
...
> 💡 **初期化機能について** > 設定画面内の「初期化（デフォルトに戻す）」ボタンを押すと、いつでも出荷時のデフォルト設定（プレフィックス・サフィックス設定等）に戻すことができます。

---

## 4. 基本的な使い方（テキスト送信手順）

PC側に、SecureInject本体ハードウェア（超小型キーボード）を接続し、スマホからテキストを転送する手順です。
