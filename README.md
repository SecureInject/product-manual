...
ステップ1: ZIPファイルの解凍
１．BOOTHから無料ダウンロード後、「←ダウンロード」メニューで「SecureInject_vx.x.x.zipを解凍しますか？」と表示されるので「解凍」を選択します。
２．「SecureInject_vx.x.x.zipを解凍しました。」と表示されるので、ZIPファイルの削除にチェックを入れ（任意）、「完了」を選択します。
　※これにより、アプリのインストールファイルがDownloadフォルダに解凍されます。

ステップ2: インストーラーの起動
• SecureInject_vx.x.x.apk がスマホ内部ストレージの「←ダウンロード」メニューの上部の「すべて」、もしくは「SecureInject_vx.x.x」を選択すると、SecureInject_vx.x.x.apk がアイコンと共に表示されるのでクリック（タップ）します。

ステップ3: セキュリティ許可の設定（Files by Google）
１．Files by Googleの警告で「セキュリティ上の理由からお使いのスマートフォンでは現在、この提供先からの不明なアプリをインストールすることはできません。これは[設定]で変更できます。」と表示されるので、「設定」を選択します。
２．「不明なアプリのインストール」画面に移動するので、「この提供元のアプリを許可」を有効（オン）にします。

ステップ4: インストールの完了とホーム画面への追加
１．画面にSecureInjectの「このアプリをインストールしますか？」と表示されるので、「インストール」を選択します。
２．インストールが完了したら、メニュー画面で下から上にスワイプし、アプリの一覧を開きます。SecureInjectがインストールされていますので、そのままアイコンをドラッグし、上方向にスワイプしてホーム画面に追加してください
---
